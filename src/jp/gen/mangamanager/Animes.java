...
import static jp.gen.mangamanager.Common.GENRE_ANIMES;
...
public class Animes extends ParentListView{
		...
		OnClickListener listener = new OnClickListener() {
			public void onClick(DialogInterface dialog, int which) {
				db.insert(TABLE_TITLE,
							...
							new String[]{null,edit.getText().toString(),GENRE_ANIMES});
				init();
			}
		};
		this.showAlertDialog(edit,listener);
	...
	private void init(){
		ArrayList<HashMap<String, String>> contents = getContents(TABLE_TITLE,
																	new String[] { COLUMN_ID, COLUMN_TITLE, COLUMN_GENRE },
																	COLUMN_GENRE + "=" + GENRE_ANIMES,
																	COLUMN_ID + " DESC");

		MyListAdapter adapter = getMyListAdapter(contents,
				R.layout.layout_list_top);
